...
# TMUX functions, from original author:
...
function tmux-rsc() {
    ...
    # create temporary session linked to permanent session, then attach client
    tmux new-session -d -t $1 -s $CLIENTID \; set-option destroy-unattached \; attach-session -t $CLIENTID
}

function tmux-mksc() {
    # create new permanent session with 1 window
    tmux new-session -d -s $1

    # generate a set of new windows
    tmux new-window -d -t $1
    tmux new-window -d -t $1
    # tmux new-window -d -t $1:9 -n "test_box" "ssh 156.26.10.119"
    # tmux new-window -d -t $1 "su -" \; set-window-option -t $1:1 remain-on-exit
    
    # call rsc to create a temporary session
    tmux-rsc $1
}
